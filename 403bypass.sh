...
function banner(){
	echo "Let me see this!😎"
	echo "Keep 12.0.0.1 open before starting this. 🔓"
}
function usage(){
        printf "Usage:\n"
		...
		printf "\t--SQLi\t\t\t\tMod Security & Lib Injection Bypass\n">&2
		printf '\n' >&2
		printf "ALL BYPASSES\n">&2
...
function prg(){
	...
	if [[ -z "${target}" ]];then
        printf "\n[${red}!${end}] ${yellow}No URL/PATH <scheme://domain.tld/path> given. Make sure go through the help!${end}\n\n"
        usage
        exit 127
    fi
	if [[ -z "${mode}" ]];then
        printf "\n[${red}!${end}] ${yellow}No mode given. Make sure go through the help${end}\n\n"
        usage
        exit 127
    fi
